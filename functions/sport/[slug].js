...
export async function onRequestGet(context) {
  ...
  try {
      ...
      return `
        <div class="card">
          ...
          <div class="text-muted mb-10">
            ...
            ${ascent.web_link ? ' • <a href="' + ascent.web_link + '" class="web-link" target="_blank">View Source</a>' : ''}
          </div>
          <a href="/athletes/${athleteSlug}" class="link">View Athlete →</a>
        </div>
      `;
    }).join('');
    ...
    const html = generateBaseHeader(climb.name, climb.name) + 
      `
        <a href="/sport" class="back-link">← Back to Sport Climbs</a>
